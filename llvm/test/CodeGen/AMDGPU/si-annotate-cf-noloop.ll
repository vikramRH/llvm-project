...
; RUN: opt -mtriple=amdgcn-- -S -passes=structurize-cfg,si-annotate-control-flow -simplifycfg-require-and-preserve-domtree=1 %s | FileCheck -check-prefix=OPT %s
; RUN: llc -mtriple=amdgcn -verify-each -simplifycfg-require-and-preserve-domtree=1 < %s | FileCheck -check-prefix=GCN %s
